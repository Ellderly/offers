...
const settingObjForm = {
  postParams: {
    ...
    _setParams: function (answers) {
      ...
      this.email = document.querySelector('input[name="email"]').value.toLowerCase();
      this.country_code.toUpperCase();
      this.comment = commentVal(answers);
      this.user_id = document.querySelector('input[name="user_id"]').value
    },
  },
};
...
const lastNameG = document.querySelectorAll('input[name="lastname"]'),
...
function itiFlagsAdd(element) {
  var iti = intlTelInput(element, {
    ...
    excludeCountries: ["am", "ge", "kz", "kg", "md", "ua", "ru", "pl", "uz"],
  ...
  settingObjForm.postParams.country_code = iti.getSelectedCountryData().iso2;
  // console.log(  settingObjForm.postParams)
...
const formLastName = document.querySelectorAll('input[name="lastname"]');
...
const postData = async (data) => {
  ...
  // preloaderFormSend();
  const apiLink = `./api.php${window.location.search}`;
  ...
  if (result.status) {
    ...
    setTimeout(() => {
      thenkYouPage(result?.link_auto_login);
    }, 2000);
...
allForm.forEach((form) => {
  form.addEventListener("click", (e) => {
    const currentForm = e.target.closest("form");
    const lastName = currentForm.querySelector('input[name="lastname"]');
    const firstName = currentForm.querySelector('input[name="name"]');
    const emailF = currentForm.querySelector('input[name="email"]');
    ...
    emailF.addEventListener("keyup", (e) => {
      validEmail(emailG);
    });
  });
  ...
  form.addEventListener("submit", async(e) => {
    ...
    const _mcObject = {};
    (new FormData(form)).forEach((value, key) => _mcObject[key] = value);
